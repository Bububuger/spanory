...
## 目标
根据最新审阅补齐 context 处理上报 OTLP 的字段契约，冻结 `agentic.context.*` 命名空间，避免后续实现时字段命名、稳定性和 emit 语义漂移。

## 方案
1. 在 `telemetry/field-spec.yaml` 中新增 `agentic.context.*` 字段组，并为 context canonical events 定义 `emit_when`、`stability`、`runtime_support`。
2. 在 `agentic-fields.md` 中补 context 字段设计说明，并把 `agentic.event.category` 扩展为支持 `context`。
3. 更新变更台账与阶段 `todo.md`，记录本次 schema freeze 的背景、影响和验证。

## 验收
- `field-spec.yaml` 中存在冻结的 `agentic.context.*` 字段定义。
- `agentic-fields.md` 能解释这些字段为何存在、如何在 OTLP 中投影。
- 文档校验通过，且不影响当前 notify 修复代码工作面。
>>>>>>> 9e3ae05 (Update README.md)
