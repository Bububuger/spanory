...
describe('BDD setup command', () => {
  it('Given fake home with stale codex notify setup, When setup apply+teardown runs for claude+codex, Then watch mode is idempotent and codex notify config is restored', async () => {
    ...
    let doctor;
    try {
      doctor = execFileSync('node', [entry, 'doctor', '--runtimes', 'claude-code,codex', '--home', fakeHome], {
        encoding: 'utf-8',
    ...
    const teardown = execFileSync('node', [entry, 'uninstall', '--runtimes', 'codex', '--home', fakeHome], {
      encoding: 'utf-8',
      env: { ...process.env, HOME: fakeHome },
    });
    ...
    const teardownAgain = execFileSync('node', [entry, 'uninstall', '--runtimes', 'codex', '--home', fakeHome], {
      encoding: 'utf-8',
      env: { ...process.env, HOME: fakeHome },
    });
