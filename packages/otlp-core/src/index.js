...
export function parseOtlpHeaders(input) {
    if (!input || !input.trim())
        return undefined;
    const pairs = input
        .split(',')
    ...
    for (const event of events) {
        ...
        if (context && event.category === 'turn') {
            ...
            if (!context.hasTurnEvent) {
                ...
                if (!context.traceInput && eventInput)
                    context.traceInput = eventInput;
                if (!context.traceOutput && eventOutput)
                    context.traceOutput = eventOutput;
            }
        }
    }
...
export async function sendOtlpHttp(endpoint, payload, headers = {}) {
