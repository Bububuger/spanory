...
function mergeUsage(target = {}, usage) {
    ...
    const cacheWrite = toNumber(usage.cacheWrite ?? usage.cache_creation_input_tokens);
    if (input !== undefined)
        next.input = (next.input ?? 0) + input;
    if (output !== undefined)
        next.output = (next.output ?? 0) + output;
    if (total !== undefined)
        next.total = (next.total ?? 0) + total;
    if (cacheRead !== undefined)
        next.cacheRead = (next.cacheRead ?? 0) + cacheRead;
    if (cacheWrite !== undefined)
        next.cacheWrite = (next.cacheWrite ?? 0) + cacheWrite;
    return next;
}
function normalizeToolCategory(name) {
    ...
    if (lower === 'mcp' || lower.startsWith('mcp__') || lower.startsWith('mcp-'))
        return 'mcp';
    if (n === 'Task')
        return 'agent_task';
    return 'tool';
...
function buildToolAttributes(name, toolCallId) {
    ...
    if (name.startsWith('mcp__')) {
        ...
        if (server)
            attrs['agentic.mcp.server.name'] = server;
    }
    return attrs;
}
...
function detectOpenclawRuntimeVersion() {
    const envVersion = normalizeRuntimeVersion(process.env.OPENCLAW_RUNTIME_VERSION
...
function extractPromptMetadata(prompt) {
    ...
    if (!match)
        return { prompt: input, attributes: {} };
    const attributes = {};
    try {
...
export function createOpenclawSpanoryPluginRuntime(logger) {
    ...
    const getState = (ctx, options = {}) => {
        ...
        if (requireResolved && !hasResolvedAlias)
            return null;
        ...
        if (ctx?.sessionKey)
            sessionAliases.set(ctx.sessionKey, stateKey);
        return state;
    };
    const findFallbackState = (ctx) => {
        const candidates = [];
        for (const state of sessions.values()) {
            if (ctx?.agentId && state.projectId !== ctx.agentId)
                continue;
    ...
    const getToolState = (ctx) => {
        ...
        if (ctx?.sessionKey)
            sessionAliases.set(ctx.sessionKey, fallback.stateKey);
        return fallback;
    };
    const seenToolCall = (state, toolCallId) => {
        ...
        if (state.seenToolCallOrder.length > 500) {
            ...
            if (evicted)
                state.seenToolCallIds.delete(evicted);
        }
        return false;
    };
    ...
    const flushCurrentBatch = (state) => {
        ...
        if (!state.currentBatch || !state.currentBatch.length)
            return;
    ...
    const enqueueToolEvent = (state, toolEvent) => {
        state.lastTouchedAt = Date.now();
        const hasPendingUnmaterializedTurn = Boolean(state.pendingTurnId
    ...
    const onLlmInput = (event, ctx) => {
        ...
        const runtimeVersion = normalizeRuntimeVersion(event?.runtimeVersion
    ...
    const onAfterToolCall = (event, ctx) => {
        ...
        if (!toolCallId)
            return;
        if (seenToolCall(state, toolCallId))
            return;
